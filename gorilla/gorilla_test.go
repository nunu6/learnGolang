...
func TestJsonHandler(t *testing.T) {
	...
	assert.Equal("Ryan", list[1].Name)
	assert.Equal("DH", list[2].Name)
}
...
func TestJsonHandler2(t *testing.T) {
	...
	err = json.NewDecoder(res.Body).Decode(&student)
	assert.Nil(err)
	assert.Equal("Ryan", student.Name)
}
...
func TestJsonHandler3(t *testing.T) {
	...
	res = httptest.NewRecorder()
	req = httptest.NewRequest("GET", "/students/4", nil) // ❸ 추가된 학생 데이터
	mux.ServeHTTP(res, req)
	assert.Equal(http.StatusOK, res.Code)
	err := json.NewDecoder(res.Body).Decode(&student)
	assert.Nil(err)
...
func TestJsonHandler4(t *testing.T) {
	...
	assert.Nil(err)
	assert.Equal(2, len(list)) // ❹ 결과 확인
	assert.Equal("Ryan", list[0].Name)
}
